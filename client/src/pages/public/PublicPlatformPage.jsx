    ...
    {isLoading ? <TableSkeleton rows={6} /> : !profiles.length ? <Card><EmptyState icon={Icon} title={`No ${config.name} profiles yet`} description={`Add students' ${config.name} usernames in the admin panel to fetch their public activity.`} /></Card> : mode === 'compare' ? <div className="space-y-6">
      <Card><div className="flex items-center gap-3 mb-5"><HiOutlineArrowsRightLeft className="w-7 h-7 text-primary-500" /><div><h2 className="text-xl font-bold text-surface-900 dark:text-white">Compare {config.name} Profiles</h2><p className="text-sm text-surface-500">Choose two students for a side-by-side comparison.</p></div></div>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-4 mb-4">
          ...
          <Select label="Filter Year" placeholder="All Years" value={year} onChange={(event) => { setYear(event.target.value); setFirstId(''); setSecondId(''); }} options={years.map((value) => ({ value: String(value), label: String(value).toLowerCase().includes('year') ? value : `Year ${value}` }))} />
        </div>
        <div className="grid md:grid-cols-[1fr_auto_1fr] gap-4 items-end"><Select label="Student A" placeholder="Select student" value={firstId} onChange={(event) => setFirstId(event.target.value)} options={options.filter((item) => item.value !== secondId)} /><span className="hidden md:block pb-3 font-bold text-surface-500">VS</span><Select label="Student B" placeholder="Select student" value={secondId} onChange={(event) => setSecondId(event.target.value)} options={options.filter((item) => item.value !== firstId)} /></div></Card>
      {first && second && <Card><div className="grid grid-cols-[minmax(100px,1fr)_1fr_1fr] gap-2 items-center"><div /><div className="text-center"><Avatar src={first.avatar_url} name={first.name} size="md" className="mx-auto" /><p className="font-bold mt-2">{first.name}</p></div><div className="text-center"><Avatar src={second.avatar_url} name={second.name} size="md" className="mx-auto" /><p className="font-bold mt-2">{second.name}</p></div>{config.metrics.map(([label, key]) => { const a = getMetrics(first, platform)[key] || 0; const b = getMetrics(second, platform)[key] || 0; return <div className="contents" key={key}><p className="text-sm text-surface-500 py-3 border-t">{label}</p><p className={`text-lg font-bold text-center py-3 border-t ${a >= b ? 'text-green-600' : ''}`}>{a}</p><p className={`text-lg font-bold text-center py-3 border-t ${b >= a ? 'text-green-600' : ''}`}>{b}</p></div>; })}</div></Card>}
    </div> : mode === 'leaderboard' ? <Card padding={false} className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-surface-50 dark:bg-surface-800 text-sm text-surface-500"><tr><th className="px-5 py-4">Rank</th><th className="px-5 py-4">Student</th>{config.metrics.map(([label, key]) => <th key={key} className="px-5 py-4">{label}</th>)}</tr></thead><tbody>{profiles.map((student, index) => { const metrics = getMetrics(student, platform); return <tr key={student.id} onClick={() => navigate(profilePath(student))} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); navigate(profilePath(student)); } }} role="button" tabIndex={0} className="border-t border-surface-200 dark:border-surface-700 hover:bg-surface-50 dark:hover:bg-surface-800/40 cursor-pointer"><td className="px-5 py-4 text-lg font-bold text-primary-500">#{index + 1}</td><td className="px-5 py-4"><div className="flex items-center gap-3"><Avatar src={student.avatar_url} name={student.name} size="sm" /><button type="button" onClick={(event) => { event.stopPropagation(); navigate(profilePath(student)); }} className="font-semibold text-left text-surface-900 dark:text-white hover:text-primary-500">{student.name}</button></div></td>{config.metrics.map(([, key]) => <td key={key} className="px-5 py-4">{metrics[key] ?? 0}</td>)}</tr>; })}</tbody></table></div></Card> : <div className="space-y-6">
      ...
      {/* Top stat cards removed as requested */}
      {platform !== 'leetcode' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4">
          ...
          <div className="space-y-4">
            <DailyTasksCard className="max-w-md ml-auto w-full" date={todayTasks?.date} problems={todayTasks?.problems} onOpenHistory={() => setHistoryOpen(true)} />
